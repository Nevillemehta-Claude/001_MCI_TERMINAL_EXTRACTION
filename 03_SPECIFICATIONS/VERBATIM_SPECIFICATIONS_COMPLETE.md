...
- Scanner Checks: Kite Token, API Connectivity, Market Status, 4 backends (ICICI Direct, HDFC Sky, Kotak Neo, Zerodha Kite), Position Sync, Order Queue, Claude AI, Time Sync, WebSocket. Status icons: ✅ PASS, ⚠️ WARN, ❌ FAIL, 🔄 RUNNING, ⏳ PENDING. Checks run in parallel, 5000ms individual timeout. Continuing.
- 12 checks → Proceed to Ignition → Select backends → Dashboard with real-time telemetry → End Session → Return to Phase 0. Error Recovery Flows: Token Validation Failed (WHAT/WHY/HOW error → Try Again → Input cleared), Scanner Check Failed (red highlight → Ignition disabled → Re-run Scanner). Continuing.
...
- 12 checks in <500ms) → Phase 2 Ignition (select 1-4 backends) → Phase 3 Dashboard (6 panels: Token Status, Backend Health, Positions, Orders, P&L Summary, AI Observations). Continuing.
- 12 checks hit CIA-SIE-PURE) → Select Backend (Paper) → ARM → IGNITE → Telemetry Dashboard with WebSocket and real data → Monitor Positions/Orders/Activity Log. Continuing.
...
- Scanner Check Failed (red highlight → Ignition disabled → Re-run Scanner). Continuing.
...
- 12 Checks Pass? │ Engine Started? │ Shutdown Complete?         │
- 12 checks in <500ms, Pass/Warning/Fail states, UXMI ProgressBar
- 12-Point Checks)      │─────┤                        │    │
- 12 Checks  │ │      │ │ Ignition   │ │                    │
- pre-ignition checks for Indian markets (P3 Remediation)
- 12-point pre-ignition checks                         │
- 12 checks) → All Pass ↓ | Token Expired → Phase 0
...
- Shutdown Sequence            (CR-002: 6 Steps)               ✅                   │
...
- Step 1: Embed all atomic facts as vectors                                 │
- Step 3: Check for ALIGNMENT                                               │
- Step 3: Check for CONTRADICTION despite similarity                        │
- Step 2: Compute similarity                                                │
- shutdownStore   │ ← CR-002: 6-step graceful shutdown                     │
- shutdownStore.ts   │ CR-002    │ ✅ 6-step shutdown          │          │
- Graceful Shutdown | 6-step DEFAULT_STEPS | shutdownStore.ts:47-78 | ✅ |
...
- Graceful Shutdown | 6-step sequence in shutdownStore.ts | ✅ 100% |
- STEP 5: Build Telemetry Dashboard (positions, health gauges, P&L)
- Graceful Shutdown | 6-step sequence | shutdownStore.ts | ✅ 100% |

---

## 6 QUALITY GATES - ALL REFERENCES

- GATE 1) → 5. Technology Selection (GATE 2 CRITICAL) → 6. Detailed Specification (GATE 3) → 7. Implementation → 8. Testing → 9. Integration → 10. Verification (GATE 4 FINAL) → 11. Deployment
...
- GATES:  Token Valid? │ 12 Checks Pass? │ Engine Started? │ Shutdown Complete?         │
- Gate Conditions: Explicit authorization required, no bypass, silence is not consent
- Gate 0 → Phase by phase → Generate indices → Final verification → Report completion
...
- Gate 0.5 - UI/UX Review                                      │
- Gate 0.5 → Phase 4 Architecture Design → Phase 6 Development
...
- GATE 0 — Awaiting authorization to proceed to Phase 1
...
- Phase 0 Token Capture (Kite login, callback URL paste, validate token) → Phase 1 Scanner (12 checks in <500ms) → Phase 2 Ignition (select 1-4 backends) → Phase 3 Dashboard (6 panels: Token Status, Backend Health, Positions, Orders, P&L Summary, AI Observations). Continuing.
...
- Phase 0. Error Recovery Flows: Token Validation Failed (WHAT/WHY/HOW error → Try Again → Input cleared), Scanner Check Failed (red highlight → Ignition disabled → Re-run Scanner). Continuing.
- Phase 6 alignment check captured. Phase 7: Implementation - **NO TECH CHANGES PERMITTED**. Critical rule: STOP → Document → Return to Phase 5 → New TSR → Resume. Phase 8: Testing begins.
- 12-Phase Gate Execution Model — 12 sequential phases with mandatory authorization gates, no parallel execution, no skippable phases
...
- Phase 0) → Configuration (Phase 1) → Sequential Ignition (Phase 2) → Operational Monitoring (Phase 3)
- 12-phase lifecycle status, 6 quality gates, 5 CRs, folder structure, UXMI components, checkpoint log
- Phase 2 Execution Sequence — Read → Analyze → Classify → Log → Migrate → Verify → Update → Report
- Phase 0 → Gate 0 → Phase by phase → Generate indices → Final verification → Report completion
...
- PHASE 1: LOAD → PHASE 2: PROCESS → PHASE 3: COMMIT → PHASE 4: RELEASE → PHASE 5: REPORT
...
- Phase 3.5 (UXMI) → Phase 0 → Phase 1 → Phase 2 → Phase 3 → Phase 4 → Production
...
- 12-Phase Gate Execution Model directives                              ║
...
- Phase 1: Pre-Ignition Scanner ⏳ PENDING → **CLAUDE CODE**
...
- POST /all route with real ciaSieClient.runHealthScan(). Map CIA-SIE response to MCI format (checkId→id, passed→status, durationMs→duration). Overall status based on critical checks. Error handling with Sentry.captureException. Add environment variables starting. Continuing.
...
- │  │                              /api/auth                                             │  │
- │  │                              /api/telemetry                                        │  │
- { type: 'zerodha', name: 'Zerodha Kite', endpoint: '/api/backend/zerodha', icon: '🪁' },
...
- GET /pnl                                                                         │  │
- POST /emergency                                                                  │  │
- { type: 'icici', name: 'ICICI Direct', endpoint: '/api/backend/icici', icon: '🏦' },
- GET /status                                                                      │  │
- GET /orders                                                                      │  │
- GET /backends                                                                    │  │
- GET /health                                                                      │  │
...
- POST /validate                                                                   │  │
- POST /start                                                                      │  │
- { type: 'kotak', name: 'Kotak Neo', endpoint: '/api/backend/kotak', icon: '🏢' },
- { type: 'hdfc', name: 'HDFC Sky', endpoint: '/api/backend/hdfc', icon: '🏛️' },
